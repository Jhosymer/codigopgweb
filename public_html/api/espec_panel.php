...
$cacheFile = 'sync/cache_espec_panel.json';
$cacheTime = 21600;
...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    // 3. Ejecutar lógica
    if ($esPeticionGeneral) {
        $lista = $_espec_panel->listarespec_panel();
        file_put_contents($cacheFile, json_encode($lista, JSON_UNESCAPED_UNICODE));
        echo json_encode($lista, JSON_UNESCAPED_UNICODE);
        http_response_code(200);
    } 
    elseif (isset($_GET['id'])) {
        echo json_encode($_espec_panel->listarespec_panelId($_GET['id']), JSON_UNESCAPED_UNICODE);
    } 
    elseif (isset($_GET['codigo'])) {
        echo json_encode($_espec_panel->listarespec_panelCodigo($_GET['codigo']), JSON_UNESCAPED_UNICODE);
    } 
    elseif (isset($_GET['total'])) {
        echo json_encode($_espec_panel->listarespec_panelTotal(), JSON_UNESCAPED_UNICODE);
    }
