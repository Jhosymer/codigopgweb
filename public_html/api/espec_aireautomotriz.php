...
$cacheFile = 'sync/cache_espec_aireautomotriz.json';
$cacheTime = 21600;
...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if ($esPeticionGeneral) {
        $lista = $_espec_aireautomotriz->listarespec_aireautomotriz();
        file_put_contents($cacheFile, json_encode($lista, JSON_UNESCAPED_UNICODE));
        echo json_encode($lista, JSON_UNESCAPED_UNICODE);
        http_response_code(200);
    } 
    elseif (isset($_GET['id'])) {
        echo json_encode($_espec_aireautomotriz->listarespec_aireautomotrizId($_GET['id']), JSON_UNESCAPED_UNICODE);
    } 
    elseif (isset($_GET['codigo'])) {
        echo json_encode($_espec_aireautomotriz->listarespec_aireautomotrizCodigo($_GET['codigo']), JSON_UNESCAPED_UNICODE);
    } 
    elseif (isset($_GET['total'])) {
        echo json_encode($_espec_aireautomotriz->listarespec_aireautomotrizTotal(), JSON_UNESCAPED_UNICODE);
    }
