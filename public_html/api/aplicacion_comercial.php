...
require_once 'clases/aplicacion_comercial.class.php';
...
$_aplicacion_comercial = new aplicacion_comercial;
...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
   ...
   /* if (!isset($_GET['token']) || $_GET['token'] !== 'Hlcbdep22') {
    ...
    if ((count($_GET) == 1 && isset($_GET['token'])) || empty($_GET)) {
        $listaDeaplicacion_comercial = $_aplicacion_comercial->listaraplicacion_comercial();
        header("Content-Type: application/json");
        echo json_encode($listaDeaplicacion_comercial);
