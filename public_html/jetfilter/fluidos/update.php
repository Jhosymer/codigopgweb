<?php 
    ...
    if( isset($_POST['codigo']) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            $argumentos_filtro_codificacion = [$codigo, $codigo_buscar, $codigo_barra, $id_tipo, $fecha_actualiza, $sincronizado, $fecha_updated, $id_codigo];

            try {
                //Se crearan los registros en la tabla de fluidos y de filtro codificación.
                ...
                $actualizando = $base_de_datos->prepare("UPDATE filtro_codificacion SET codigo = ?, codigo_buscar = ?, codigo_barra = ?, id_tipo = ?, fecha_actualiza = ?, sincronizado = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
