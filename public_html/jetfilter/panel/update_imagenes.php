<?php 
    /* -----ARCHIVO PARA ACTUALIZAR LAS IMAGENES UN REGISTRO EN PANEL-------- */
    ...
    if( isset( $_POST['btnimg'] ) ){
        ...
        //Datos a editar en la tabla de panel
        ...
        try {
            //Se crearan los registros en la tabla de panel y de filtro codificación.
            //En caso de que falle alguna subida, se cancelara todo
            $base_de_datos->beginTransaction();

            $seleccionado = $base_de_datos->prepare("UPDATE espec_panel SET sincronizado = ?, imagen = ?, imagen1 = ?, imagen2 = ?, imagen3 = ?, updated_at = ?  WHERE id = ?");
    ...
    //Si se pulson un boton para eliminar imagenes
        ...
        try {
            ...
            if( $_POST['imagen_eliminada'] == 'button-eliminar-imagen1' ){ 
                $sql = "UPDATE espec_panel SET sincronizado = :sincronizado, imagen = '' WHERE id = :id";
            }
            else if( $_POST['imagen_eliminada'] == 'button-eliminar-imagen2' ){
                $sql = "UPDATE espec_panel SET sincronizado = :sincronizado, imagen1 = '' WHERE id = :id";
            }
            else if( $_POST['imagen_eliminada'] == 'button-eliminar-imagen3' ){
                $sql = "UPDATE espec_panel SET sincronizado = :sincronizado, imagen2 = '' WHERE id = :id";
            }
            else if( $_POST['imagen_eliminada'] == 'button-eliminar-imagen4' ){
                $sql = "UPDATE espec_panel SET sincronizado = :sincronizado, imagen3 = '' WHERE id = :id";
            }
