        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h1 class="titulo">Nuevo Usuario Administrativo</h1>
                    <form action="index.php" method="post" class="mb-0">
                        <button type="submit" class="btn btn-secondary" name="volver">Atrás</button>
                    </form>
                </div>
                <div class="card-body">
                    <form action="crub.php" method="post" onsubmit="return validarClaves_nuevo()">
                        ...
                        <a class="mb-3 btn btn-outline-primary" style="width: fit-content; text-align: left;" data-bs-toggle="collapse" href="#collapsePermisos-nuevo" role="button" aria-expanded="true" aria-controls="collapsePermisos-nuevo">
                            <i class='bx bx-check-square'></i> Permisos de Función
                        </a>
                        
                        <div class="collapse show" id="collapsePermisos-nuevo">
                            <div class="row mb-3">
                                <?php
                                $count = 0;
                                foreach ($permisos_disponibles as $permiso) {
                                    ...
                                    // Se quitó la validación para que el ID 1 no aparezca seleccionado por defecto.
                                    $checked = '';
                                    $is_toggle_all = ($permiso_id == 1) ? 'onchange="toggleAll(this)"' : '';
                                    $checkbox_id = ($permiso_id == 1) ? 'seleccionarTodos-nuevo' : 'permiso-' . $permiso_id . '-nuevo';
