<?php 
    /* -----ARCHIVO PARA GUARDAR UN REGISTRO EN AIRE INDUSTRIAL-------- */
    ...
    if( isset( $_POST['codigo'] ) ){
        ...
        //Datos a guardar en la tabla de aire industrial
        $argumentos = [$max_id_codigo, $codigo, $codigo_buscar, $tipo, $diametro_ext, $diametro_ext2, $diametro_int, $diametro_int2, $altura, $detalle1, $detalle2, $sincronizado, $imagen[0], $imagen[1], $imagen[2], $imagen[3]];
        ...
        try {
            //Se crearan los registros en la tabla de aire industrial y de filtro codificación.
            //En caso de que falle alguna subida, se cancelara todo
            $base_de_datos->beginTransaction(); 
            
            $sql = "INSERT INTO espec_aireindustrial (id_codigo, codigo, codigo_buscar, tipo, diametroext1, diametroext2, diametroint1, diametroint2, altura, detalle1, detalle2, sincronizado, imagen, imagen1, imagen2, imagen3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $seleccionado = $base_de_datos->prepare($sql);
            $seleccionado->execute($argumentos);

            $sql = "INSERT INTO filtro_codificacion (id, clase, codigo, codigo_buscar, id_tipo, filtracion, descripcion, precio, und_empaque, fecha_actualiza, codigo_barra, sincronizado, deleted_at, updated_at) VALUES (?, 'aireindustrial', ?, ?, ?, ?, ?, 0, ?, ?, ?,?, null, null)";
            $seleccionado = $base_de_datos->prepare($sql);
            ...
            //Si todo va bien
            $_SESSION['nuevo'] = true;
            header("location: ./espec_aireindustrial.php") or die('Error');
        }
