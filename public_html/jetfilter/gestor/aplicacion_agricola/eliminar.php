...
date_default_timezone_set('America/Caracas');
    ...
    if( !isset($_POST['id']) ){
        header("location: ./aplicacion_agricola.php");
    }
    session_start();
    ...
    try {
        ...
        $eliminar_aplicacion_agricola = $base_de_datos->prepare("UPDATE aplicacion SET deleted_at = :fecha 
                                                    WHERE id = :id") or die('Error al eliminar');
        $eliminar_aplicacion_agricola->bindParam(':fecha', $fecha, PDO::PARAM_STR);
        $eliminar_aplicacion_agricola->bindParam(':id', $id, PDO::PARAM_STR);
        $eliminar_aplicacion_agricola->execute();
