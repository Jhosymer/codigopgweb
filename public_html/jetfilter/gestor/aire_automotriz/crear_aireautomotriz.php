    ...
    if( isset( $_POST['codigo'] ) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            try {
                ...
                $sql = "INSERT INTO espec_aireautomotriz (id_codigo, codigo, codigo_buscar, tipo, diametroext1, diametroext2, diametroint1, diametroint2, altura, detalle1, detalle2, sincronizado, imagen, imagen1, imagen2, imagen3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $seleccionado = $base_de_datos->prepare($sql);
                $seleccionado->execute($argumentos);

                $sql = "INSERT INTO filtro_codificacion (id, clase, codigo, codigo_buscar, id_tipo, filtracion, descripcion, precio, und_empaque, fecha_actualiza, codigo_barra, sincronizado,  deleted_at, updated_at) VALUES (?, 'aireautomotriz', ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, null, null)";
                $seleccionado = $base_de_datos->prepare($sql);
                ...
                //Si todo va bien
                $_SESSION['nuevo'] = true;
                header("location: ./espec_aireautomotriz.php") or die('Error');
            }
