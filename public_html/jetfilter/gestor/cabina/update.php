<?php 
    ...
    if( isset($_POST['codigo']) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            $ancho = $_POST['ancho'];
            $altura = $_POST['altura'];
            $und_empaque = ( $_POST['und_empaque'] == '' ) ? 'N/D' : $_POST['und_empaque'];
            ...
            //Datos a guardar en la tabla de cabina
            $argumentos = [$codigo, $codigo_buscar, $tipo, $largo, $ancho, $altura, $detalle1, $detalle2, $sincronizado, $imagen[0], $imagen[1], $imagen[2], $imagen[3], $fecha_updated, $id];
            //Datos a guardar en la tabla de filtro codificación
            $argumentos_filtro_codificacion = [$codigo, $codigo_buscar, $id_tipo, $filtracion, $und_empaque, $fecha_actualiza, $codigo_barra, $sincronizado, $fecha_updated, $id_codigo];
            ...
            try {
                ...
                $actualizando = $base_de_datos->prepare("UPDATE espec_cabina SET codigo = ?, codigo_buscar = ?, tipo = ?, largo = ?, ancho = ?, altura = ?, detalle1 = ?, detalle2 = ?, sincronizado = ?, imagen = ?, imagen1 = ?, imagen2 = ?, imagen3 = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
                $actualizando->execute($argumentos);
                
                $actualizando = $base_de_datos->prepare("UPDATE filtro_codificacion SET clase = 'cabina', codigo = ?, codigo_buscar = ?, id_tipo = ?, filtracion = ?, und_empaque = ?, fecha_actualiza = ?, codigo_barra = ?, sincronizado = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
