...
$columnas = ['tipos.id', 'tipos.tipo', 'categorias.categoria', 'categorias.clase', 'productos.nombre'];
...
if($campo != null){
    ...
    $where .= ") and (tipos.deleted_at is null)";
}
else {
    $where = "WHERE ( tipos.deleted_at is null )";
}
...
$sql = "SELECT " . implode(',',$columnas) ."
                FROM tipos
                JOIN categorias ON categorias.id = tipos.categoria_id
                JOIN productos ON productos.id = categorias.producto_id
                WHERE ( tipos.deleted_at is null )";
