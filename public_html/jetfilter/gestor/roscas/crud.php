...
$codigo_critico = preg_replace('/\s+/', ' ', trim($codigo_critico)); // Colapsa múltiples espacios en uno solo
    try {
        ...
        $check_sql = "SELECT id FROM roscas 
        ...
        if ($id) {
            $sql = "UPDATE roscas SET codigo = :codigo, valor_nominal = :valor, updated_at = NOW() WHERE id = :id";
            $stmt = $base_de_datos->prepare($sql);
            $stmt->bindParam(':id', $id);
            $status = "actualizado";
        } else {
            $sql = "INSERT INTO roscas (codigo, valor_nominal, updated_at, deleted_at) VALUES (:codigo, :valor, NOW(), NULL)";
            $stmt = $base_de_datos->prepare($sql);
            $status = "nuevo";
        }
