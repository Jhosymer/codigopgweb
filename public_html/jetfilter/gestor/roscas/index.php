<?php
   ...
   $paginas = [10, 25, 50, 100];
    ...
    alerta_codigo_existe("Ya existe una rosca con un código similar");
?>

    <div class="d-flex justify-content-between align-items-center mt-5 mb-2">
        <div class="text-center flex-grow-1">
            <h1 class="titulo">Roscas</h1>
        </div>
...
function mostrarFormulario() {
    ...
    const modo = document.getElementById('id_rosca').value === "" ? "Nueva Rosca" : "Editar Rosca";
    document.getElementById('modalRoscasLabel').innerText = modo;
...
function abrirModal(id, codigo, valor, modo) {
    const modalElement = document.getElementById('modalRoscas');
    const modal = new bootstrap.Modal(modalElement);
    const form = document.getElementById('formModalRoscas');
    
    // Reiniciar vista al Paso 1
    document.getElementById('contenedorAlerta').classList.remove('d-none');
    document.getElementById('contenedorFormulario').classList.add('d-none');
    document.getElementById('footerFormulario').classList.add('d-none');

    // TÍTULO DINÁMICO: Si es Ver, va directo. Si es Nuevo/Editar, muestra "Verificación"
    if (modo === 'ver') {
        document.getElementById('modalRoscasLabel').innerText = "Detalles de la Rosca";
        mostrarFormulario(); 
    } else {
        document.getElementById('modalRoscasLabel').innerText = "Verificación Obligatoria";
    }

    form.action = "crud.php";
    document.getElementById('id_rosca').value = id;
