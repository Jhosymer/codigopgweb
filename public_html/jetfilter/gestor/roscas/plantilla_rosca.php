<?php 
    ...
    // 1. Columnas que se van a mostrar (ajustadas a la tabla roscas)
    ...
    // 2. Consultas usando la tabla 'roscas'
    $sql_total = "SELECT id FROM roscas WHERE (deleted_at IS NULL)";
    $busqueda_total = $base_de_datos->prepare($sql_total);
    $busqueda_total->execute();
    $filas_totales = $busqueda_total->rowCount();

    $sql_filtrado = "SELECT id, codigo, valor_nominal FROM roscas $where";
    ...
    $sql_datos = "SELECT id, codigo, valor_nominal FROM roscas $where ORDER BY valor_nominal ASC $sLimit";
