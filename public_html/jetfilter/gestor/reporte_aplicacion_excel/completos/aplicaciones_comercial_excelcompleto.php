<?php
   ...
   // mb_internal_encoding('UTF-8');
        ...
        if ( ($tipo == 'todo' ) or ($tipo == 'Comercial' )){
            if ($marcaset == '' ){
				...
				$seleccionadoc = $base_de_datos->prepare("SELECT a.id_tipo, a.id_marca, a.id_vehiculo, a_v.modelo, a_v.motor, a_v.ano, a_m.marca, count(*) 
																							...
																							WHERE ( a.id_tipo = 2 ) and ( a.deleted_at is null ) and ( a_v.deleted_at is null )  
				...
				$seleccionadoc_esperac = $base_de_datos->prepare("SELECT a.codigo, a.aplicacion FROM aplicacion as a 
																								JOIN aplicacion_marca as a_m ON a.id_marca = a_m.id 
																								JOIN aplicacion_vehiculo as a_v ON a.id_vehiculo = a_v.id 
																								WHERE ( a.id_tipo = 2 ) and ( a.deleted_at is null ) and ( a_v.deleted_at is null )  and a.id_vehiculo = ? and a.id_marca = ?
																								GROUP BY a.aplicacion, a.codigo");
				
				$seleccionadoc = $base_de_datos->prepare("SELECT a.aplicacion FROM aplicacion as a 
															WHERE ( a.id_tipo = 2 ) and ( a.deleted_at is null )  
...
<table>
    <thead>
        <tr><th><h1 style=' color: #E2001A;'>Aplicacion Comercial </h1></th></tr>
    </thead>
</table>
...
<?php
				...
				$seleccionadoc = $base_de_datos->prepare("SELECT a.id_tipo, a.id_marca, a.id_vehiculo, a_v.modelo, a_v.motor, a_v.ano, a_m.marca, count(*) 
																							...
																							JOIN aplicacion_marca as a_m ON a.id_marca = a_m.id 
																							JOIN aplicacion_vehiculo as a_v ON a.id_vehiculo = a_v.id 
																							WHERE ( a.id_tipo = 2 ) and ( a.deleted_at is null ) and ( a_v.deleted_at is null )  and  a.id_marca = '$marcaset'
																							GROUP BY a.id_tipo, a.id_marca, a.id_vehiculo, a_v.modelo, a_m.marca 
																							ORDER BY a_m.marca, a_v.modelo");
				$seleccionadoc->execute();
				while ($filac = $seleccionadoc->fetch(PDO::FETCH_ASSOC)) {
				...
				$seleccionadoc = $base_de_datos->prepare("SELECT a.aplicacion FROM aplicacion as a 
															WHERE ( a.id_tipo = 2 ) and ( a.deleted_at is null )   and  a.id_marca = '$marcaset'
															GROUP BY a.aplicacion
															ORDER BY a.aplicacion");
                    ...
                    $consulta = $base_de_datos->prepare("SELECT * FROM `aplicacion` WHERE id_tipo = 2 and id_marca = :marcaset and updated_at is null");
...
<table style=" width:100%; border-collapse: collapse; text-align: left;">
    ...
    <table>
    <thead>
        <tr><th><h3 >No Hay Registro En Aplicacion Comercial Para la Marca Seleccionada </h3></th></tr>
    </thead>
</table> <?php
}
