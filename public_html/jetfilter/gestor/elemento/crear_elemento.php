<?php 
    /* -----ARCHIVO PARA AGREGAR UN REGISTRO EN ELEMENTO-------- */
    ...
    if( isset( $_POST['codigo'] ) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            $argumentos = [$max_id_codigo, $codigo, $codigo_buscar, $tipo, $diametro_ext, $diametro_int, $diametro_int2, $altura, $detalle1, $detalle2, $sincronizado, $imagen[0], $imagen[1], $imagen[2], $imagen[3]];
            ...
            try {
                ...
                $sql = "INSERT INTO espec_elemento (id_codigo, codigo, codigo_buscar, tipo, diametroext1, diametroint1, diametroint2, altura, detalle1, detalle2, sincronizado, imagen, imagen1, imagen2, imagen3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $seleccionado = $base_de_datos->prepare($sql);
                $seleccionado->execute($argumentos);

                $sql = "INSERT INTO filtro_codificacion (id, clase, codigo, codigo_buscar, id_tipo, filtracion, descripcion, precio, und_empaque, fecha_actualiza, codigo_barra, sincronizado, deleted_at, updated_at) VALUES (?, 'elemento', ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, null, null)";
                $seleccionado = $base_de_datos->prepare($sql);
                ...
                $_SESSION['nuevo'] = true;
                header("location: espec_elemento.php") or die('Error');
            }
