<?php 
    /* ARCHIVO PARA CREAR Y LLENAR LA TABLA DE MARCAS DE APLICACIONES */
    ...
    $sql = "SELECT id, marca, sincronizado FROM aplicacion_marca
    ...
    if($output['totalFiltro'] > 0){
        ...
        $output['paginacion'] .= "";
        if($page != 1){
            $output['paginacion'] .= "<button class='btn_borde_rweb_form' onclick='getMarcas(1)'><<</button>"; 
        }
        for($i = $numeroInicio; $i <= $numeroFinal; $i++){
            if($page == $i){
               $output['paginacion'] .= "<button class='btn_rweb_form'>".$i."</button>";
                ...
                $output['paginacion'] .= "<button class='btn_borde_rweb_form' onclick='getMarcas($i)'>".$i."</button> ";
            }
        }
        if($page != $totalPaginas){
            $output['paginacion'] .= "<button class='btn_borde_rweb_form' onclick='getMarcas($totalPaginas)'>>></button> "; 
        }
    }
