<?php 
    ...
    if( !isset($_POST['id']) ){
        header("location: espec_panel.php");
    }
    else {
        $id = $_POST['id'];
    }
    ...
    if( isset($_POST['codigo']) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            //Datos a guardar en la tabla de panel
            ...
            try {
                ...
                $actualizando = $base_de_datos->prepare("UPDATE espec_panel SET codigo = ?, codigo_buscar = ?, tipo = ?, largo = ?, ancho = ?, altura = ?, detalle1 = ?, detalle2 = ?, sincronizado = ?, imagen = ?, imagen1 = ?, imagen2 = ?, imagen3 = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
                $actualizando->execute($argumentos);
                
                $actualizando = $base_de_datos->prepare("UPDATE filtro_codificacion SET clase = 'panel', codigo = ?, codigo_buscar = ?, codigo_barra = ?, id_tipo = ?, filtracion = ?, und_empaque = ?, fecha_actualiza = ?, sincronizado = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
