<?php 
    /* -----ARCHIVO PARA ACTUALIZAR UN REGISTRO EN ELEMENTO-------- */
    ...
    if( isset($_POST['codigo']) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            $argumentos = [$codigo, $codigo_buscar, $tipo, $diametro_ext, $diametro_int, $diametro_int2, $altura, $detalle1, $detalle2, $sincronizado, $imagen[0], $imagen[1], $imagen[2], $imagen[3], $fecha_updated, $id];
            //Datos a guardar en la tabla de filtro codificación
            $argumentos_filtro_codificacion = [$codigo, $codigo_buscar, $id_tipo, $filtracion, $und_empaque, $fecha_actualiza, $sincronizado, $fecha_updated, $id_codigo];
            ...
            try {
                ...
                $actualizando = $base_de_datos->prepare("UPDATE espec_elemento SET codigo = ?, codigo_buscar = ?, tipo = ?, diametroext1 = ?, diametroint1 = ?, diametroint2 = ?, altura = ?, detalle1 = ?, detalle2 = ?, sincronizado = ?, imagen = ?, imagen1 = ?, imagen2 = ?, imagen3 = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
                $actualizando->execute($argumentos);
                
                $actualizando = $base_de_datos->prepare("UPDATE filtro_codificacion SET codigo = ?, codigo_buscar = ?, id_tipo = ?, filtracion = ?, und_empaque = ?, fecha_actualiza = ?, sincronizado = ?, updated_at = ?  WHERE id = ?") or die("Error al actualizar");
