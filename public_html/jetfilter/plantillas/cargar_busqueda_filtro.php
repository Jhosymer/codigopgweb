...
$columnas = ['id', 'codigo_busq', 'respuesta', 'ciudad', 'estado', 'fecha' ];
...
$sql = "SELECT " . implode(', ', $columnas) ." FROM busqueda_filtro ";
...
if($filas_filtradas > 0){
    while( $row = $busqueda->fetch() ){
        ...
        for($i = 0; $i < (count($columnas)); $i++){
            ...
            if( $columnas[$i] == 'respuesta' && $row[$i] == 1 ){
                $output['data'] .= "<td>SI</td>";
            }
            else if( $columnas[$i] == 'respuesta' && $row[$i] == 0 ){
