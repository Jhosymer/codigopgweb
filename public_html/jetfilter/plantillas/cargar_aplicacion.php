<?php 
    ...
    if($output['totalFiltro'] > 0){
        ...
        $output['paginacion'] .= "";
        if($page != 1){
            $output['paginacion'] .= "<a onclick='getData(1, `$tipo`)'  style='cursor: pointer;'>Primero</a>"; 
        }
        for($i = $numeroInicio; $i <= $numeroFinal; $i++){
            ...
            $output['paginacion'] .= "<p>" . $i ." </p>";
                ...
                $output['paginacion'] .= "<a onclick='getData($i, `$tipo`)'  style='cursor: pointer;'>".$i."</a>";
            }
        }
        if($page != $totalPaginas){
            $output['paginacion'] .= "<a onclick='getData($totalPaginas, `$tipo`)'  style='cursor: pointer;'>Último</a>"; 
        }
    }
