...
$campo = $_POST["codigo_p"] ?? $_POST["codigo_pr"] ?? "";
$html = "";
$primer_match_encontrado = false;

if (!empty($campo)) {
    // Consulta con OR para validar act_sap en ambas tablas como pediste
    ...
    while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
        // Lógica de resaltado (Active)
        $clase_activa = '';
        if (!$primer_match_encontrado) {
            if (stripos($row['codigo'], $campo) === 0) {
                $clase_activa = 'active'; 
        ...
        $item_html = "<li class='list-group-item $clase_activa' style='cursor:pointer;' 
                        ...
                        <div class='d-flex align-items-center'>
                            ...
                            <span><b>{$row['codigo']}</b> -- {$row['descripcion']}</span>
                        </div>
                      </li>";
