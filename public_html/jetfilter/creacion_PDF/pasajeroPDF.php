...
$seleccionado = $base_de_datos->prepare("SELECT a.id_tipo, a.id_marca, a.id_vehiculo, a_v.modelo, a_v.motor, a_v.ano, a_m.marca, count(*) 
                                                                        ...
                                                                        WHERE ( a.id_tipo = 1 ) and ( a.deleted_at is null ) and ( a_v.deleted_at is null )
...
$seleccionado_espera = $base_de_datos->prepare("SELECT a.codigo, a.aplicacion FROM aplicacion as a 
                                                                                JOIN aplicacion_marca as a_m ON a.id_marca = a_m.id 
                                                                                JOIN aplicacion_vehiculo as a_v ON a.id_vehiculo = a_v.id 
                                                                                WHERE ( a.id_tipo = 1 ) and ( a.deleted_at is null ) and ( a_v.deleted_at is null ) and a.id_vehiculo = ? and a.id_marca = ?
                                                                                GROUP BY a.aplicacion, a.codigo");

$seleccionado = $base_de_datos->prepare("SELECT a.id_tipo, a.id_marca, a.id_vehiculo, a.aplicacion, a.codigo 
                                                                                        FROM aplicacion as a 
                                                                                        JOIN aplicacion_marca as a_m ON a.id_marca = a_m.id 
                                                                                        JOIN aplicacion_vehiculo as a_v ON a.id_vehiculo = a_v.id 
                                                                                        WHERE ( a.id_tipo = 1 ) and ( a.deleted_at is null ) and ( a_v.deleted_at is null )
                                                                                        ORDER BY a_m.marca, a_v.modelo, a.aplicacion");
$seleccionado->execute();
while ($fila = $seleccionado->fetch(PDO::FETCH_ASSOC)) {
    if( $fila['aplicacion'] == "Hidraulico primario" ){
        $fila['aplicacion'] = "Hidráulico primario";
    }
    else if($fila['aplicacion'] == "Hidraulico"){
        $fila['aplicacion'] = "Hidráulico";
    }
    else if($fila['aplicacion'] == "Hidraulico secundario"){
        $fila['aplicacion'] = "Hidráulico secundario";
    }
    $aplicacion_codigos []= $fila;
}

$seleccionado = $base_de_datos->prepare("SELECT a.aplicacion FROM aplicacion as a 
                                            WHERE ( a.id_tipo = 1 ) and ( a.deleted_at is null )
...
<body>
    ...
    <table>
        <thead>
           <tr > <th  class="titulo"  colspan="5"><h3>Vehiculos Livianos</h3></th>
              <th class="fecha" width= "20Px" colspan="4"><img src="<?php echo $base64; ?>"></th>
            </tr>

              <tr><th class="fecha" colspan="9"><?php echo "$fechaActual";?></th> </tr>
            <tr>
                <th><h6>Modelo</h6></th>
                <th><h6>Motor</h6></th>
                <th><h6>Año</h6></th>
...
unset($aplicacion_codigos);
    ...
    $domPDF->render();
    $pdf = $domPDF->output();
    file_put_contents("./../PDF/vehiculos_pasajeros.pdf",$pdf);
