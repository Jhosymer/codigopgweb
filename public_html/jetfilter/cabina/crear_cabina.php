    ...
    if( isset( $_POST['codigo'] ) ){
        ...
        if( $numeroFiltros == 0 ){
            ...
            try {
                ...
                $sql = "INSERT INTO espec_cabina (id_codigo, codigo, codigo_buscar, tipo, largo, ancho, altura, detalle1, detalle2, sincronizado, imagen, imagen1, imagen2, imagen3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
                $seleccionado = $base_de_datos->prepare($sql);
                $seleccionado->execute($argumentos);

                $sql = "INSERT INTO filtro_codificacion (id, clase, codigo, codigo_buscar, id_tipo, filtracion, descripcion, precio, und_empaque, fecha_actualiza, sincronizado, deleted_at, updated_at) VALUES (?, 'cabina', ?, ?, ?, ?, ?, 0, ?, ?, ?, null, null)";
                $seleccionado = $base_de_datos->prepare($sql);
